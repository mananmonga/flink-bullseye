...
- Anything that changes an operator uid, a state name or a metric name is a breaking change and
  needs a major version. `BarrierUids` and `docs/design.md` say why.
...
- Keep `barrier-core` free of Kafka types and both modules free of `implementation` dependencies on
  Flink.
